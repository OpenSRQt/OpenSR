...
ResourceReply::ResourceReply(const QUrl &url, QIODevice *device, QObject *parent)
    : QNetworkReply(parent), m_device(device)
{
...
ResourceManagerNAM::ResourceManagerNAM(ResourceManager *manager, QObject *parent)
    : QNetworkAccessManager(parent), m_manger(manager)
{
}
...
QNetworkReply *ResourceManagerNAM::createRequest(Operation op, const QNetworkRequest &req, QIODevice *outgoingData)
{
    ...
    if ((scheme.compare("res", Qt::CaseInsensitive) == 0) ||
        (scheme.compare("dat", Qt::CaseInsensitive) == 0) && (op == QNetworkAccessManager::GetOperation))
    {
...
{
public:
    ResourceManagerNAMFactory(ResourceManager *manager) : QQmlNetworkAccessManagerFactory(), m_manager(manager)
    {
    }
...
ResourceInfo::ResourceInfo() : provider(0)
{
}
...
ResourceManager::ResourceManager(QObject *parent) : QObject(parent), m_namFactory(new ResourceManagerNAMFactory(this))
{
}
...
{
    if (path.scheme().compare("res", Qt::CaseInsensitive) && path.scheme().compare("dat", Qt::CaseInsensitive))
    {
        return 0;
    }
...
PGKResourceInfo::PGKResourceInfo(PKGItem *item_) : ResourceInfo(), item(item_)
{
}
...
PKGIODevice::PKGIODevice(const PKGItem &item, QIODevice *archiveDev, QObject *parent) : QIODevice(parent), m_buffer(0)
{
...
QIODevice *PKGProvider::getDevice(const ResourceNode &node, QObject *parent)
{
    ...
    PGKResourceInfo *info = dynamic_cast<PGKResourceInfo *>(node.info.data());
...
void PKGProvider::load(ResourceNode &current, PKGItem *item)
{
    for (int i = 0; i < item->childCount; i++)
    {
        ...
        QString lowerName = QString(static_cast<char *>(child->name)).toLower();
        if (child->dataType == 3)
        {
            ResourceNode node(static_cast<char *>(child->name), ResourceNode::DIRECTORY, &current);
        ...
        {
            ResourceNode node(static_cast<char *>(child->name), ResourceNode::FILE, &current,
